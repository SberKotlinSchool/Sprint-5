...
fun Shop.allOrderedProducts(): Set<Product> = customers.flatMap { it.orders.flatMap { value -> value.products } }.toSet()
...
fun Shop.getNumberOfDeliveredProductByCity(): Map<City, Int> =
    customers
        .groupBy { it.city }
        .mapValues { (_, customersInCity) ->
            customersInCity.sumOf { customer ->
                customer.orders.filter { it.isDelivered }.sumOf { it.products.size }
            }
        }
...
fun Shop.getProductsOrderedByAll(): Set<Product> =
    customers
        .map { it.orders.flatMap { value -> value.products }.toSet() }
