...
fun getSumWithIndexDivisibleByThree(list: List<Long>): Long {
    return list.asSequence().withIndex().filter { it.index % 3 == 0 }.sumOf { it.value }
}
...
fun Shop.getCustomerWithMaximumNumberOfOrders(): Customer? =
    customers.map { Pair(it.orders.count(), it) }.maxByOrNull { it.first }?.second
...
fun Shop.getProductsOrderedByAll(): Set<Product> =
    customers.fold(allOrderedProducts()) { acc, customer -> acc.intersect(customer.products()) }

fun Customer.products() = orders.flatMap { it.products }.toSet()
