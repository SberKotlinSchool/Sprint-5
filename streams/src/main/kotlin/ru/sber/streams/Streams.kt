...
fun Shop.allOrderedProducts(): Set<Product> =
    this.customers.flatMap { customer -> customer.orders.flatMap { order -> order.products } }.toSet()

// 5. Получить покупателя, который сделал больше всего заказов.
fun Shop.getCustomerWithMaximumNumberOfOrders(): Customer? =
    this.customers.maxByOrNull { customer -> customer.orders.size }

// 6. Получить самый дорогой продукт, когда-либо приобртенный покупателем.
fun Customer.getMostExpensiveProduct(): Product? =
    this.orders.flatMap { order -> order.products }.maxByOrNull { product -> product.price }

// 7. Получить соответствие в мапе: город - количество заказанных и доставленных продуктов в данный город.
fun Shop.getNumberOfDeliveredProductByCity(): Map<City, Int> =
    this.customers.groupBy(
        { it.city }, { it.orders }
    ).mapValues { it -> it.value.flatMap { it.filter { order -> order.isDelivered } }.sumOf { it.products.size } }

// 8. Получить соответствие в мапе: город - самый популярный продукт в городе.
fun Shop.getMostPopularProductInCity(): Map<City, Product> =
    this.customers.groupBy { customer ->
        customer.city
    }.mapValues { entry ->
        entry.value.flatMap { customer -> customer.orders }.flatMap { order -> order.products }
            .groupingBy { product -> product }.eachCount()
            .maxByOrNull { productIntEntry -> productIntEntry.value }!!.key
    }

// 9. Получить набор товаров, которые заказывали все покупатели.
fun Shop.getProductsOrderedByAll(): Set<Product> =
    this.customers.map {
        customer -> customer.orders.flatMap { order -> order.products }.toSet()
    }.reduce {
        result, products -> result.intersect(products)
    }
