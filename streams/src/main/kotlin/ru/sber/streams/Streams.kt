...
fun getSumWithIndexDivisibleByThree(list: List<Long>): Long {
    return list.withIndex()
        .filter { it.index % 3 == 0 }.sumOf { it.value }
}
...
fun Shop.getNumberOfDeliveredProductByCity(): Map<City, Int> {
    return customers
        .groupBy { it.city }
        .map { Pair(it.key, it.value.flatMap { it.orders.filter { it.isDelivered }.flatMap { it.products }}.count()) }.toMap()
}

// 8. Получить соответствие в мапе: город - самый популярный продукт в городе.
fun Shop.getMostPopularProductInCity(): Map<City, Product> {
    return customers
        .groupBy { it.city }.map{ Pair( it.key, it.value
...
fun Shop.getProductsOrderedByAll(): Set<Product> {
    return customers.fold(allOrderedProducts()) { initial, next
        -> initial.intersect(next.orders.flatMap { it.products }.toSet()) }
}
