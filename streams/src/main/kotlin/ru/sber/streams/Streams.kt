...
fun Shop.getNumberOfDeliveredProductByCity(): Map<City, Int> = customers.groupBy { it.city }.mapValues { (_, v) ->
    v.flatMap { it.orders }.filter { it.isDelivered }.sumOf { it.products.size } }

// 8. Получить соответствие в мапе: город - самый популярный продукт в городе.
fun Shop.getMostPopularProductInCity(): Map<City, Product> = customers.filter { it -> it.orders.any{ it.isDelivered } }
    .groupBy { it.city }.mapValues { (_, v) ->
        v.flatMap { it.orders }.flatMap { it.products }.groupingBy { it }.eachCount().maxByOrNull { it.value }!!.key
    }


// 9. Получить набор товаров, которые заказывали все покупатели.
fun Shop.getProductsOrderedByAll(): Set<Product> = customers.groupBy({ it }, { it -> it.orders.flatMap { it.products }})
    .mapValues { it.value.flatten().toSet() }.values
