...
fun Shop.getNumberOfDeliveredProductByCity(): Map<City, Int> {
    val deliveredProducts = mutableMapOf<City, Int>()
    customers.forEach { customer ->
        val city = customer.city
        val count = customer.orders.filter { it.isDelivered }.flatMap { it.products }.size
        deliveredProducts[city] = deliveredProducts.getOrDefault(city, 0) + count
    }
...
fun Shop.getMostPopularProductInCity(): Map<City, Product> {
    return customers.groupBy { it.city }.mapValues { entry ->
        entry.value.flatMap { it.orders }.flatMap { it.products }.groupBy { it }.maxByOrNull { it.value.size }!!.key
    }
}

// 9. Получить набор товаров, которые заказывали все покупатели.
fun Shop.getProductsOrderedByAll(): Set<Product> {
    return customers.associateBy(
        { it },
        { customer ->
